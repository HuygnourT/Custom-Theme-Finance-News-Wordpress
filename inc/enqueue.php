<?php
/**
 * Enqueue v2 - FIX: Tắt tất cả WP CSS mặc định phá layout
 */
...
add_action('wp_enqueue_scripts', function () {
    // Block editor CSS - NGUYÊN NHÂN CHÍNH phá layout
    wp_dequeue_style('wp-block-library');
    wp_dequeue_style('wp-block-library-theme');
    wp_deregister_style('wp-block-library');
    wp_deregister_style('wp-block-library-theme');

    // Global styles - thêm padding/margin vào body
...
add_action('init', function () {
    // Xoá inline CSS của blocks
    remove_action('wp_head', 'wp_enqueue_block_support_styles', 1);
}, 100);

// Tắt SVG filters mà WP thêm vào body
add_action('after_setup_theme', function () {
    remove_action('wp_body_open', 'wp_global_styles_render_svg_filters');
});

// === TẮT jQuery (tiết kiệm 90KB) ===
add_action('wp_enqueue_scripts', function () {
    if (!is_admin()) {
